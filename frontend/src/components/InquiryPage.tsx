...
export const InquiryPage: React.FC<InquiryPageProps> = ({
  ...
  // Dropdown UI states
  const [tempSelectedSchool, setTempSelectedSchool] =
    ...
    const fetchSchoolBranding = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE_URL}/schools/${selectedSchoolId}/public`,
        );

        if (res.status === 404) {
          setSchoolError(
            "This admissions form is not available. Please contact the school directly.",
          );
          return;
        }
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.contactNumber.trim()) {
      ...
      newErrors.contactNumber =
        "Invalid contact number. Use format e.g. 0821234567 or +27821234567";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/schools/${selectedSchoolId}/prospects`,
        {
          method: "POST",
          headers: {
      ...
      if (res.status === 201) {
        ...
        if (errorData.detail && typeof errorData.detail === "string") {
          ...
          setNetworkError(
            "Invalid form entry. Please check your details and try again.",
          );
        }
      } else {
      ...
      setNetworkError(
        "Something went wrong. Please try again or contact the school directly.",
      );
    } finally {
  ...
  const filteredSchools = schools.filter(
    (s) =>
      ...
      !s.schoolName.toLowerCase().includes("test"),
  );
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 sm:p-6 md:p-8">
        <div className="w-full max-w-[500px] flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-md border border-gray-150">
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
                Submit an inquiry to express initial interest and connect
                directly with a school's admissions team.
              </p>
            </div>
            ...
            <div className="relative mb-5" ref={dropdownRef}>
              <label
                ...
                className="text-slate-600 uppercase tracking-widest text-[11px] font-bold select-none mb-1.5 block"
                ...
                {loadingSchoolsList ? (
                  ...
                  <>
                    <div className="relative">
                      ...
                      />
                      <svg
                        className="absolute right-3.5 top-3.5 h-4 w-4 text-slate-400 pointer-events-none"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2.5}
            ...
            {tempSelectedSchool && (
              <div className="mb-5 p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-between animate-fade-in">
                <p className="text-xs text-blue-900 font-semibold leading-tight">
                  <span
                    className="text-[10px] uppercase text-blue-500 font-bold block mb-0.5"
                    style={{ letterSpacing: "0.5px" }}
                  >
                    Selected School
                  </span>
          ...
          <div className="text-center pb-4 text-xs font-semibold text-slate-400 tracking-wide">
            Powered by <span className="text-slate-500 font-bold">Knit</span> ·{" "}
            <a
              href="https://knit.cash"
              target="_blank"
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 sm:p-6 md:p-8">
      <div className="w-full max-w-[580px] flex flex-col gap-6 animate-fade-in">
        ...
        {(!initialSchoolId || onBackToLogin) && (
          <div className="flex justify-between items-center px-1">
            ...
            >
              ...
              <span>
                {initialSchoolId ? "Back to Login" : "Back to school selection"}
              </span>
            </button>
          </div>
        )}
          ...
          {!success ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div className="border-b border-gray-100 pb-3 mb-1">
                <p className="text-xs text-slate-500 leading-relaxed font-semibold">
                  Submit this quick inquiry form to express initial interest in{" "}
                  {school.name}. Submitting this form creates an entry with our
                  admissions department, who will get in touch with you shortly
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label
                    ...
                    className="text-gray-700 uppercase tracking-widest text-[11px] font-bold select-none"
                    style={{ letterSpacing: "0.5px" }}
                  >
                    Contact Number
                  </label>
            ...
            <div className="flex flex-col items-center text-center p-4">
              ...
              <p className="text-gray-600 text-sm leading-relaxed max-w-sm mb-8">
                ...
                admissions team will contact you within 2 business days.
              </p>
        ...
        <div className="text-center pb-4 text-xs font-semibold text-gray-400/80 tracking-wide">
          Powered by <span className="text-gray-500 font-bold">Knit</span> ·{" "}
          <a
            href="https://knit.cash"
            target="_blank"
            rel="noopener noreferrer"
