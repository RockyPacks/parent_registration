...
const DeclarationStep: React.FC<DeclarationStepProps> = ({ applicationId, onDataChange, onStepChange, onStepComplete }) => {
  ...
  const handleContinue = async () => {
      ...
      try {
        ...
        // Save to localStorage
        localStorage.setItem('declarationData', JSON.stringify(declarationData));
        console.log('Declaration data saved to localStorage');
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          <button
            ...
            disabled={isContinueDisabled || isSubmitting} // Disable while submitting
            className="w-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white py-3 px-6 rounded-lg hover:from-indigo-600 hover:to-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-sm flex items-center justify-center"
          >
            {isSubmitting ? (
              <>
