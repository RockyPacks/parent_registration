...
@app.post("/risk-check")
async def run_risk_check(data: RiskCheckRequest, current_user: dict = Depends(get_current_user)):
    try:
        ...
        # Get application_id from current user's context (from localStorage or session)
        # Since the frontend passes the applicationId in the component, we need to extract it properly
        application_id = current_user.get("application_id") or "unknown"

        # For now, implement a basic risk assessment
        # In production, this would call the Netcash Risk Reports API
        risk_score = 0.0
        flags = []
        status = "low"
        ...
        if not branch_code or not account_number:
            risk_score = 100.0
            status = "high"
            flags.append("Missing banking details")
        elif len(branch_code) != 6 or not branch_code.isdigit():
            risk_score = 85.0
            status = "high"
            flags.append("Invalid branch code format")
        elif len(account_number) < 10 or len(account_number) > 12 or not account_number.isdigit():
            risk_score = 85.0
            status = "high"
            flags.append("Invalid account number format")
        else:
            # Simulate risk assessment based on account patterns
            # In production, this would be replaced with actual API call to Netcash
            if account_number.startswith("123"):
                risk_score = 75.0
                status = "medium"
                flags.append("Account pattern requires verification")
            else:
                risk_score = 15.0
                status = "low"
                flags.append("Account verified")
        ...
        logger.info(f"Risk assessment completed for reference: {data.reference}, score: {risk_score}")
